...
class RSoccerHelper:
    ...
    def norm_pos(pos):
        return np.clip(
            pos / RSoccerHelper.MAX_POS,
            -RSoccerHelper.NORM_BOUNDS,
            RSoccerHelper.NORM_BOUNDS)

    @staticmethod
    def norm_v(v):
        return np.clip(
            v / RSoccerHelper.MAX_V,
            -RSoccerHelper.NORM_BOUNDS,
            RSoccerHelper.NORM_BOUNDS)

    @staticmethod
    def norm_w(w):
        return (w / math.pi) * RSoccerHelper.NORM_BOUNDS
    ...
    def get_attacker_observations(
        ...
        return RSoccerHelper.get_default_observations(field_data, is_left_team, robot_id)
    ...
    def get_goalkeeper_observations(
        ...
        observations = RSoccerHelper.get_ball_observations(field_data, is_left_team)

        robot = field_data.robots[robot_id]

        observations.extend(RSoccerHelper.get_own_team_robot_observations(is_left_team, robot))
    ...
    def get_own_team_robot_observations(
        ...
        def norm_pos(pos): return RSoccerHelper.norm_pos(pos)
        def norm_v(v): return RSoccerHelper.norm_v(v)
        def norm_w(w): return RSoccerHelper.norm_w(w)
    ...
    def get_default_observations(
        ...
        observation.extend(RSoccerHelper.get_ball_observations(field_data, is_left_team))

        robot = field_data.robots[robot_id]
        observation.extend(RSoccerHelper.get_own_team_robot_observations(is_left_team, robot))
        ...
        for robot in field_data.foes:
            observation.extend(RSoccerHelper.get_opponent_team_robot_observations(is_left_team, robot))
